...
object runEnsembles {
  ...
  def main(arg: Array[String]) {
    ...
    val train = sc.textFile("hdfs://hadoop-master/user/CD_76139799/train-under-100.data", 150).map(line => converter.parserToLabeledPoint(line)).persist
    ...
    //Write Results
    val writer = new PrintWriter(new File("resultsRUS.txt"))
    writer.write(
